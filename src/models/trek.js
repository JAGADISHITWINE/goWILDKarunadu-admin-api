...
const { resolveTrekUploadPath } = require("../utils/uploadPaths");
...
async function createTrek(trek, files) {
  ...
  try {
    ...
    const trekId = createUuid();

    // 🏔️ Insert trek
    await conn.query(
      `INSERT INTO treks (
        id, name, location, category, collection, difficulty,
      ...
      [
        ...
        resolveTrekUploadPath(files.coverImage[0])
      ]
    );
    ...
    if (Array.isArray(trek.highlights)) {
      for (const h of trek.highlights.filter(Boolean)) {
        await conn.query(
          `INSERT INTO trek_highlights (id, trek_id, highlight) VALUES (?, ?, ?)`,
          [createUuid(), trekId, h]
        );
      }
    }
    ...
    if (Array.isArray(trek.thingsToCarry)) {
      for (let i = 0; i < trek.thingsToCarry.length; i++) {
        ...
        if (item) {
          await conn.query(
            `INSERT INTO trek_things_to_carry (id, trek_id, item, display_order)
             VALUES (?, ?, ?, ?)`,
            [createUuid(), trekId, item, i + 1]
          );
        }
      }
    }
    ...
    if (Array.isArray(trek.importantNotes)) {
      for (let i = 0; i < trek.importantNotes.length; i++) {
        ...
        if (note) {
          await conn.query(
            `INSERT INTO trek_important_notes (id, trek_id, note, display_order)
             VALUES (?, ?, ?, ?)`,
            [createUuid(), trekId, note, i + 1]
          );
        }
      }
    }
    ...
    // 📈 Elevation Waypoints
    if (Array.isArray(trek.elevationWaypoints)) {
      for (let i = 0; i < trek.elevationWaypoints.length; i++) {
        const wp = trek.elevationWaypoints[i];
        if (wp && (wp.name || wp.elevation)) {
          await conn.query(
            `INSERT INTO trek_elevation_waypoints (id, trek_id, km, elevation, name, icon, note, display_order)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [createUuid(), trekId, Number(wp.km || 0), Number(wp.elevation || 0), wp.name || 'Checkpoint', wp.icon || '⛰️', wp.note || '', i]
          );
        }
      }
    }
    ...
    if (files.gallery?.length) {
      for (const img of files.gallery) {
        await conn.query(
          `INSERT INTO trek_images (id, trek_id, image_url) VALUES (?, ?, ?)`,
          [createUuid(), trekId, resolveTrekUploadPath(img)]
        );
      }
    }
...
async function updateTrek(trekId, trek, files) {
  ...
  try {
    ...
    if (files?.coverImage?.length) {
      coverImageValue = resolveTrekUploadPath(files.coverImage[0]);
    ...
    if (Array.isArray(trek.thingsToCarry) && trek.thingsToCarry.length > 0) {
      for (let i = 0; i < trek.thingsToCarry.length; i++) {
        ...
        if (item) {
          await conn.query(
            'INSERT INTO trek_things_to_carry (id, trek_id, item, display_order) VALUES (?, ?, ?, ?)',
            [createUuid(), trekId, item, i + 1]
          );
        }
      }
    ...
    if (Array.isArray(trek.importantNotes) && trek.importantNotes.length > 0) {
      for (let i = 0; i < trek.importantNotes.length; i++) {
        ...
        if (note) {
          await conn.query(
            'INSERT INTO trek_important_notes (id, trek_id, note, display_order) VALUES (?, ?, ?, ?)',
            [createUuid(), trekId, note, i + 1]
          );
        }
      }
    ...
    // ========== INSERT ELEVATION WAYPOINTS ==========
    await conn.query(`DELETE FROM trek_elevation_waypoints WHERE trek_id = ?`, [trekId]).catch(() => null);
    if (Array.isArray(trek.elevationWaypoints) && trek.elevationWaypoints.length > 0) {
      for (let i = 0; i < trek.elevationWaypoints.length; i++) {
        const wp = trek.elevationWaypoints[i];
        if (wp && (wp.name || wp.elevation)) {
          await conn.query(
    ...
    const existingBatchIds = existingBatches.map(b => String(b.id || '').trim()).filter(Boolean);
    ...
    if (Array.isArray(trek.batches) && trek.batches.length > 0) {
      for (let i = 0; i < trek.batches.length; i++) {
        ...
        let batchId = null;
        const requestedBatchId = String(batch.id || '').trim();
        ...
        if (batchId !== null) {
          ...
          // Delete itinerary activities before days to satisfy foreign key constraints
          await conn.query(
            `DELETE ia
             FROM itinerary_activities ia
             INNER JOIN itinerary_days idy ON idy.id = ia.day_id
             WHERE idy.batch_id = ?`,
            [batchId]
          );
...
async function insertBatchNestedData(conn, batchId, batch) {
  ...
  if (Array.isArray(batch.itineraryDays) && batch.itineraryDays.length > 0) {
    for (const day of batch.itineraryDays) {
      ...
      if (!Number.isFinite(dayNumber) || !title) {
        continue;
      }
      ...
      await conn.query(
        `INSERT INTO itinerary_days (id, batch_id, day_number, title, created_at)
         VALUES (?, ?, ?, ?, NOW())`,
        [dayId, batchId, dayNumber, title]
      );

      // Insert activities
      if (Array.isArray(day.activities) && day.activities.length > 0) {
        for (const activity of day.activities) {
          const activityTime = String(activity?.activityTime || '').trim();
          const activityText = String(activity?.activityText || '').trim();
          ...
          await conn.query(
            `INSERT INTO itinerary_activities (id, day_id, activity_time, activity_text, created_at)
             VALUES (?, ?, ?, ?, NOW())`,
            [createUuid(), dayId, activityTime, activityText]
          );
        }
      }
    }

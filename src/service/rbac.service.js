...
async function ensureRbacSchema() {
  ...
  for (const role of ROLE_DEFINITIONS) {
    if (!roleMap.has(role.roleKey)) {
      ...
      await db.query(
        "INSERT INTO admin_roles (id, role_key, role_name) VALUES (?, ?, ?)",
        [roleId, role.roleKey, role.roleName]
      );
      roleMap.set(role.roleKey, roleId);
    }
  }

  const [permissionRows] = await db.query("SELECT id, permission_key FROM admin_permissions");
  const permissionMap = new Map(permissionRows.map((row) => [row.permission_key, String(row.id || '').trim()]));

  for (const permissionKey of PERMISSIONS) {
    if (!permissionMap.has(permissionKey)) {
      const permissionId = createUuid();
      await db.query(
        "INSERT INTO admin_permissions (id, permission_key, description) VALUES (?, ?, ?)",
...
async function upsertRolePermission(roleId, permissionKey, allowed) {
  ...
  await db.query(
    `INSERT INTO admin_role_permissions (role_id, permission_id, allowed)
     VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE allowed = VALUES(allowed)`,
    [String(roleId || '').trim(), String(permission.id || '').trim(), allowed ? 1 : 0]
  );
}
  ...
  for (const row of rows) {
    ...
    for (const [groupKey, permissionKeys] of Object.entries(TABLE_GROUP_MAP)) {
      ...
      for (const permissionKey of permissionKeys) {
        await upsertRolePermission(String(role.id || '').trim(), permissionKey, allowed);
      }
    }
  }

...
async function createTrek(req, res) {
  try {
    console.log('[Upload] createTrek invoked with files', {
    ...
    console.log('[Upload] Trek create assets uploaded', {
      cover: (req.files.coverImage || []).map((file) => resolveTrekUploadPath(file)),
      gallery: (req.files.gallery || []).map((file) => resolveTrekUploadPath(file)),
    ...
    await auditService.logAuditEvent({
      actionType: 'trek.create',
      entityType: 'trek',
      entityId: trekId,
      summary: `Created trek ${trek.name || trekId}`,
      afterData: {
        name: trek.name,
        location: trek.location,
        category: trek.category,
        difficulty: trek.difficulty,
        batches: Array.isArray(trek.batches) ? trek.batches.length : 0,
      },
      metadata: {
        coverImage: !!req.files?.coverImage?.length,
        galleryImages: req.files?.gallery?.length || 0,
      },
      ...getAuditActor(req),
    });
...
async function updateTrek(req, res) {
  try {
    console.log('[Upload] updateTrek invoked with files', {
      coverImage: req.files?.coverImage?.length || 0,
      gallery: req.files?.gallery?.length || 0
    });
    const trekId = String(req.params.id || '').trim();
    ...
    if (req.files?.coverImage?.length || req.files?.gallery?.length) {
      console.log('[Upload] Trek update assets uploaded', {
        cover: (req.files.coverImage || []).map((file) => resolveTrekUploadPath(file)),
        gallery: (req.files.gallery || []).map((file) => resolveTrekUploadPath(file)),
        route: req.originalUrl
      });
    ...
    await auditService.logAuditEvent({
      actionType: 'trek.update',
      entityType: 'trek',
      entityId: trekId,
      summary: `Updated trek ${trek.name || trekId}`,
      afterData: {
        name: trek.name,
        location: trek.location,
        category: trek.category,
      ...
      metadata: {
        coverImage: !!req.files?.coverImage?.length,
        galleryImages: req.files?.gallery?.length || 0,
        coverDeleted: !!trek.coverDeleted,
        deletedGallery: Array.isArray(trek.deletedGallery) ? trek.deletedGallery.length : 0,
      },
      ...getAuditActor(req),
    });
...
async function getTreks(req, res) {
  ...
  try {
    const [treks] = await conn.execute(`
      ...
      LEFT JOIN bookings b ON tb.id = b.batch_id COLLATE utf8mb4_unicode_ci AND b.booking_status IN ('pending', 'confirmed')
      GROUP BY t.id
      ORDER BY t.created_at DESC
    `);
...
async function stopBooking(req, res) {
  ...
  try {
    ...
    await auditService.logAuditEvent({
      actionType: 'batch.stop',
      entityType: 'batch',
      entityId: batchId,
      summary: `Stopped booking for batch ${batchId}`,
      afterData: {
        batchId,
        trekName: batch[0]?.trek_name || null,
        status: 'inactive',
      },
      metadata: {
...
async function resumeBooking(req, res) {
  ...
  try {
    ...
    await auditService.logAuditEvent({
      actionType: 'batch.resume',
      entityType: 'batch',
      entityId: batchId,
      summary: `Resumed booking for batch ${batchId}`,
      afterData: {
        batchId,
        trekName: batch[0]?.trek_name || null,
        status: 'active',
...
async function exportBookings(req, res) {
  ...
  try {
    ...
    const fileName = `${batch.trek_name.replace(/\s+/g, '_')}_${formatDateOnlyForMySQL(batch.start_date).replace(/-/g, '')}_Bookings.xlsx`;
...
async function exportallBookings(req, res) {
  ...
  try {
    ...
    // Set response headers
    const fileName = `${trek.name.replace(/\s+/g, '_')}_All_Bookings_${formatDateOnlyForMySQL(new Date()).replace(/-/g, '')}.xlsx`;
